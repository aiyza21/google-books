...
const renderBooks = (volume) => {
    ...
    output.title = volume.volumeInfo?.title ?? "Title not available.";
    output.authors = volume.volumeInfo?.authors ?? "Author not available.";
    output.description =
        volume.volumeInfo?.description ?? "Description not available.";
    output.image = volume.volumeInfo.imageLinks.thumbnail;

    // Book Container created
    const div = document.createElement("div");
    div.classList.add("books");
    document.body.appendChild(div);

    // img element being created
